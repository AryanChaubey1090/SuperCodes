...
// Temporary members staged while creating a new trip
let pendingTripMembers = [];
...
function saveTrip() {
  ...
  // Save any members added inline during trip creation
  pendingTripMembers.forEach(m => {
    state.members.push({ id: uid(), name: m.name, email: m.email, phone: '', tripId: trip.id, createdAt: Date.now() });
  });
  pendingTripMembers = [];
...
// ============ INLINE MEMBER (inside Trip Modal) ============
function addInlineMember() {
  const nameEl = document.getElementById('inlineMemberName');
  const emailEl = document.getElementById('inlineMemberEmail');
  const name = nameEl.value.trim();
  const email = emailEl.value.trim();

  if (!name) { showToast('Please enter a member name!', 'error'); return; }
  if (pendingTripMembers.some(m => m.name.toLowerCase() === name.toLowerCase())) {
    showToast('This member is already added!', 'error'); return;
  }

  pendingTripMembers.push({ name, email });
  nameEl.value = '';
  emailEl.value = '';
  renderPendingMembers();
  nameEl.focus();
...
function removeInlineMember(index) {
  pendingTripMembers.splice(index, 1);
  renderPendingMembers();
}
...
function renderPendingMembers() {
  ...
  if (!list) return;
  if (!pendingTripMembers.length) {
    list.innerHTML = '<p class="no-inline-members">No members added yet.</p>';
    return;
  }
  list.innerHTML = pendingTripMembers.map((m, i) => `
    <div class="inline-member-chip">
      <div class="inline-member-avatar">${getInitials(m.name)}</div>
      <div class="inline-member-info">
        <span class="inline-member-name">${m.name}</span>
        ${m.email ? `<span class="inline-member-email">${m.email}</span>` : ''}
      </div>
      <button type="button" class="inline-member-remove" onclick="removeInlineMember(${i})" title="Remove">✕</button>
    </div>`).join('');
}
...
function populatePaidBy() {
  ...
  // Members assigned to this specific trip shown first, then everyone else
  const tripMembers = state.members.filter(m => m.tripId === tripId);
  const otherMembers = state.members.filter(m => m.tripId !== tripId);
  const allMembers = [...tripMembers, ...otherMembers];

  sel.innerHTML = '<option value="">-- Select Member --</option>'
    + tripMembers.map(m => `<option value="${m.id}">${m.name}</option>`).join('')
    + (otherMembers.length ? `<optgroup label="Other Members">${otherMembers.map(m => `<option value="${m.id}">${m.name}</option>`).join('')}</optgroup>` : '');

  if (!allMembers.length) {
    splitDiv.innerHTML = '<p style="color:var(--muted);font-size:0.85rem">No members yet. Add members from the Members page.</p>';
    return;
  }

  splitDiv.innerHTML = allMembers.map(m => `
    <label class="check-label">
      <input type="checkbox" value="${m.id}" ${m.tripId === tripId || !tripId ? 'checked' : ''}/>
      <span>${m.name}${m.tripId !== tripId && m.tripId ? ' <em style="color:var(--muted);font-size:0.78em">(other trip)</em>' : ''}</span>
    </label>`).join('');
}
